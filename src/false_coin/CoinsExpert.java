...
class CoinsExpert {
    ...
    private static final int NUMBER_OF_COINS = 12;
    private int[] arr = new int[NUMBER_OF_COINS];
    CoinsExpert(){
        Arrays.fill(arr,STANDARD_WEIGHT);
        changeOneToFake();
    }

    private void changeOneToFake(){
        Random random = new Random();
        arr[random.nextInt(NUMBER_OF_COINS)] = (random.nextInt()>0 ? STANDARD_WEIGHT+1 : STANDARD_WEIGHT-1);
    }
    ...
    void setFalseCoin(int index, int value){
        Arrays.fill(arr,STANDARD_WEIGHT);
        if (index > 11 || index < 0){
            System.out.println("Index is not correct.");
            return;
        }
        if (value == STANDARD_WEIGHT){
        ...
        if (sumOfCoins(arr[0],arr[1],arr[2],arr[3]) > sumOfCoins(arr[4],arr[5],arr[6],arr[7])){
            ...
            if (sumOfCoins(arr[0],arr[1],arr[2],arr[4],arr[5]) > sumOfCoins(arr[8],arr[9],arr[10],arr[11],arr[3])){
                ...
                if (arr[0] == arr[1]){
                    ...
                    System.out.println("Weight of the false coin: "+ arr[2]+" (for checking)");
                }
                else {
                    System.out.println("Result step 3: coin "+(arr[0] > arr[1] ? 1 : 2)+" more than another one, and it's fake.");
                    System.out.println("Weight of the false coin: "+ (arr[0] > arr[1] ? arr[0] : arr[1])+" (for checking)");
                }
            }
                ...
                if (arr[4] == arr[5]){
                    ...
                    System.out.println("Weight of the false coin: "+ arr[3]+" (for checking)");
                }
                else {
                    System.out.println("Result step 3: coin "+(arr[4] < arr[5] ? 5 : 6)+" less than another one, and it's fake.");
                    System.out.println("Weight of the false coin: "+ (arr[4] < arr[5] ? arr[4] : arr[5]) +" (for checking)");
                }
            }
            else{
                ...
                System.out.println("Weight of the false coin: "+ (arr[6] < arr[7] ? arr[6] : arr[7]) +" (for checking)");
            }
        }
            ...
            System.out.println("Result step 1: First (1,2,3,4) and second parts (5,6,7,8) have the same weight. It means third part (9,10,11,12) has a fake coin.");
            ...
            if (sumOfCoins(arr[8], arr[9], arr[10]) > sumOfCoins(arr[0], arr[1],arr[2])) {
                ...
                if (arr[8] == arr[9]) {
                    ...
                    System.out.println("Weight of the false coin: "+ arr[10] +" (for checking)");
                } else {
                    System.out.println("Result step 3: coin "+(arr[8] > arr[9] ? 9 : 10)+" more than another one, and it's fake.");
                    System.out.println("Weight of the false coin: "+ (arr[8] > arr[9] ? arr[8] : arr[9]) +" (for checking)");
                }
                ...
                System.out.println("Weight of the false coin: "+ arr[11] +" (for checking)");
            }
        }
    }
}
